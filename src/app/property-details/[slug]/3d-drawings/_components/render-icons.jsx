// Icons used only by the 3D Drawings page — the archive download, the
...
// nothing outside it reads from here, and nothing here is shared with the
// Blueprints or Photos folders, which each keep their own copies for the same
// reason.
//
// Same convention as the shared sets: currentColor everywhere, so a badge
// recolours from its own ground rather than from a baked-in fill.
//
// Drawn on a 16×16 artboard — these sit in buttons and badges at 12–20px,
...
// Stamped on every visual the studio modelled. On this page a render is the
// norm rather than the exception — the Photos gallery marks the render and
// leaves the photograph unmarked, and here it runs the other way — so this is
// the stamp most tiles carry. Solid rather than stroked: it sits at 12px on a
// translucent badge, where a 1.8 stroke breaks up.
export const ComputerGeneratedIcon = (props) => (
...
// The counterpart stamp: a frame of the house as built, standing in the set
// beside the visual it was modelled from.
